...
- README.md 项目介绍文件
- public 该文件下存在的是项目的 html 模版,意思就是你写的所有的 vue 代码其实都是以该模版为基础的(模板变了，整个项目也就变了)，一般不需要修改，偶尔可能会引入一些文件(css js 等工具类的)
- public 下的 index.html 中的 <noscript>标签作用：当浏览器不支持 js 的时候，该标签生效；否则不生效。
- babel.config.js babel 的配置文件，babel 是一个 js 编译工具，作用是编译新版本的 js 语法以及某些 vue 语法，以让浏览器识别 。
- src 文件夹是 vue 项目的源代码
  - assets(该文件夹名称必须是 assets，里面可以随便建) 存储静态文件的文件夹，一般存放一些 公共的 css 图片等
  - components 存放 vue 组件的，该文件夹名称可以修改
...
删除 App 里 和 HelloWorld 与 img 相关的东西
App.vue 只剩下
...
- 在父组件内的导出对象内使用 components 属性先注册子组件。
      ...
      export default {
        ...
        // 组件注册
        components: {
        // 前边的 Header 是标签名，后边的 Header 是组件名
        // Header: Header,
          Header,
        },
      }
...
接收完毕之后需要在子组件的 template 中使用，使用方式分为两大类
...
- 带数据的(有 data)
  <!-- item in arrList 就相当于遍历数组 arrList, item 代表每一项,想要使用下标的话  (item,index) in arrList ，(item 和 index 可以改名，第二个参数index是索引值，从0开始)-->
  <!-- key 属性是必须的 而且该循环下值是唯一的 -->
  ```html
...
- `v-show` 样式的消失和出现 display:none; 和 display:block;
...
reduce((最终结果，数组的每一项)=>{在这里面返回一个新的对象，当作下一次循环的初始值，直到循环完毕结束之后，就是最终的结果},最终结果的初始值)
...
- methods 将函数设置成普通函数，该函数的根作用域(就是一级)下的 this 就是组件实例.如果根作用域内的其他子作用域想要直接访问 this 那么请设置成箭头函数 (因为箭头函数在定义的时候，this 指向就已经确定好了。而 function 函数在调用的时候才知道 this 指向，而且 this 所在的也不是根作用域。)
- computed(计算属性) 跟上面 methods 一样
- 生命周期函数跟上面一样
...
- beforeCreate
- created: data 等其他组件的数据处理完毕,可以在页面初始的时候发送请求更新数据。发送请求使用的是 axios 插件。 axios 会自动把请求到的数据放到 data 属性中。
- beforeMount
