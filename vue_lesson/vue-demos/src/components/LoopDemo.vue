  ...
  <div>
    <div class="wrap">
      ...
      <button @click="change(3)" :class="ind === 3 ? 'active' : ''">4</button>
        ...
        <!-- 事件修饰符  .native 的作用是直接给组件的根元素的 dom 节点绑定事件，是给组件使用的 -->
        <!-- 组件标签内的属性都被当做 props 中的东西 -->
        <!-- 1. props  比如：text="0" -->
        <!-- 2. 自定义事件  比如：@click.native="change(4)"-->
    ...
    <!-- 使用 v-for 指令, 必须加上key 属性(key 必须跟在 v-for 指令标签内) ,该属性的属性值在当前的 for 内必须唯一 -->
    <!--  -->
    <ul>
