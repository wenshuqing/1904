...
- ssh 和 http 的区别：输入密码和不输入密码。(别的都不会影响)
...
一个仓库内可以有多个分支，默认只有一个分支 master ，通常称为主分支(用来放合并后的代码)。也可以创建无数个其他分支。一般工作流程是先在主分支将项目的主体框架搭建完毕，然后创建多个分支(创建的分支内容和 master 一样)，每个分支代表不同的功能，不同的程序猿分别在不同分支内进行开发，开发完毕将代码合并到主分支。

- 在网上创建一个仓库(breach_demo)带上 readMe ,然后克隆到本地，添加上公共的 css 和 js 然后上传。
...
- 需要将创建好的分支上传到网上，可能直接使用 `git push` 就会成功,但是由于网上并没有新分支，所以需要使用 `git push --set-upstream origin yourbranch` 上传。     
  - 注：(1)`origin` 是起的一个远端名，可以是任意的；(2)当上传是网上`yourbranch`分支时，需要使用此命令。
...
- 主分支分别合并两个分支的内容，然后上传。需要使用 `git merge yourbranch` 命令合并分支。合并之后上传。
  - 当合并之后修改 a 分支，再上传 a 的时候：当没有冲突时会自动跳出留言板(按shift+Z+Z即可退出)；当有冲突时，需要选择保留哪个(是网上的还是本地的)。而不管有没有冲突命令行都会暂停执行等待用户操作的原因是，本地有网上没有的，网上有本地没有的。即使是先拉取网上的在上传可能也不行，因为这种情况可能永远存在(本地有网上没有的，网上有本地没有的，其实就是 a或b 有 master 没有的，master 有 a或b 中没有的)，毕竟有多个分支。然后就慢慢的去解决问题就好了。

#### github 的特殊分支 gh-pages
- 部署
  - 拿我们创建好的 branch_demo 仓库来说，mater 分支内已经存放好了其他分支做好的项目(项目成品，包括 index...),github 仓库有一个特殊的分支叫 gh-pages ,该分支下的内容会被自动托管到 github 免费服务器(也就是说只要该分支下有 index.html 就可以直接使用网址访问)
   `其实就是只需要直接创建 gh-pages 分支，并上传到网上即可`。
- 部署完毕之后在网上查看的网址：(1)wenshuqing.github.io/某个仓库名; (2)wenshuqing.github.io/某个仓库名/此仓库下的某个页面名
即`你的名字/仓库名/某个页面名(index.html可以省略)`
...
- `git branch` 显示出各个分支名字
- `git checkout yourbranch` 切换分支
- `git merge yourbranch` 当前分支合并分支其他分支，注意：为了避免问题，合并分支的时候最好一个个的合并。
...
- 仓库不能嵌套仓库
- 版本回退
- 当多个人或者多台电脑管理一个仓库时，先执行`git pull` 将网上的更新拉取下来，再做更改，避免版本冲突
- 在网上创建仓库的时候，会自动放到 master 分支下
- 在本地操作的时候，一个仓库只能看到一个分支下的内容
- 当本地的东西和远端的东西一样时，就不需要`git add .`和`git commit`了，只需要`git push`

#### 类似 github 网站的其他工具

- 腾讯云开发者平台,是一个和 github 类似的网站，服务器是国内的操作起来很快。注册一下就行。`DevOps项目模板`有部署功能，任何一个分支部署静态网站都可以，并不像github一样只有gh-pages分支可以部署。
